...
var unifycoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in unifycoreDefaults)
    if (unifycoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = unifycoreDefaults[i];
